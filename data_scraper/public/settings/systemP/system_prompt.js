...
async function system_prompt(name = "default") {
    ...
    try {
        ...
        const response = await fetch(`/api/system_instructions/${encodeURIComponent(qs)}/${encodeURIComponent(database_name)}`);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    if(newPrompt === data) {
        ...
        try {
            const response = await fetch('/api/set_custom_instruction', {
                ...
                body: JSON.stringify({ name: location, instructions: newPrompt, database_name: database_name})
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
        } catch (error) {
